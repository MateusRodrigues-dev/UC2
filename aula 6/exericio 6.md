...
Header — nome/logo do produto e uma frase de efeito (tagline).
Nav — menu de navegação, com links (por âncora ou entre páginas, sua escolha) pras seções/páginas do site.
Seção "Sobre" — apresentação do produto/serviço (o que é, pra quem é, por que interessa).
Seção de multimídia — pelo menos um recurso: imagem, vídeo ou áudio, com boas práticas (alt, dimensões definidas).
Tabela — comparando algo relevante ao seu produto (ex.: planos, versões, especificações). Deve ter cabeçalho definido e pelo menos uma célula mesclada (colspan ou rowspan).
Formulário — de captura de contato/interesse (ex.: "quero saber mais", lista de espera, orçamento). Use pelo menos 4 tipos diferentes de campo (text, email, select, radio, checkbox, textarea, etc.).
Footer — informações de rodapé (nome do produto, ano, links úteis).
